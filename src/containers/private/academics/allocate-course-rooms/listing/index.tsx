...
const CourseRoomsListing: FC = () => {
  ...
  useEffect(() => {
    getAllCourseAllocatedRooms(pagination.page, search);
  }, []);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (response.isConfirmed) {
      ...
      // deleteCourseAllocatedRoom(id, setData, queryParams, setPagination);
    }
  };
  ...
  const onPageChange = (pageInfo: { selected: number }) => {
    ...
    getAllCourseAllocatedRooms(page + 1, search);
  };
  const getAllCourseAllocatedRooms = (page: number, search: string) => {
    const queryParams = {
      per_page: pagination.per_page,
      page,
      search,
  ...
  const goToEditCourseAllocatedRoom = (id: number) => {
