...
import {
  SuperRoleManagementCreateMain,
  SuperRoleManagementCreateTop,
  Form,
  SuperRoleManagementCreateSection,
} from "./style";
...
const RollManagementCreate: FC<AdmissionStudentListingProps> = ({ }) => {
  ...
  const onSubmit = () => {
    ...
    if (params?.id) {
      updateRoleBySuperAdmin(params?.id, formData);
    } else {
      createRoleBySuperAdmin(formData);
    }
  };
  ...
  useEffect(() => {
    if (params?.id)
      getRoleByIdBySuperAdmin(params?.id, formData, setValue, setFormData);
  }, []);
  ...
  return (
    <SuperRoleManagementCreateMain>
      ...
      <SuperRoleManagementCreateSection className="content-radius-shadow">
